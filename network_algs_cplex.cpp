...
void NAC::readprob(int nnn) {
	...
	////Create CPLEX related variables for populating network flow problem.
	...
	//Setup the supply and demand to replicate Shortest path problem.
	...
	//Now, on tree, retrace path backward from t to s.
	clock_t time7 = clock();
	cst = 0;
	int currentnd = T;
	printf("%d ", currentnd);
	...
	//Doing modified problem by reading the next M lines.
